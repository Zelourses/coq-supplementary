...
Proof.
  destruct id1 as [n]. destruct id2 as [m].
  remember (eq_nat_dec n m) as Rem.
  inversion Rem.
  - left. rewrite H. reflexivity.
  - right. injection. assumption. (* well.. *)
Qed.

Lemma eq_id : forall (T:Type) x (p q:T), (if id_eq_dec x x then p else q) = p.
Proof.
  intros.
  remember (id_eq_dec x x) as Rem.
  destruct Rem.
  - reflexivity.
  - contradiction.
Qed.
...
Proof. 
  ...
  inversion H. inversion H0. subst. lia.
Qed.
...
Proof. 
  ...
  - right. inversion_clear H1. apply gt_conv. lia.
  - left. apply H2.
